...
<div>
    ...
    <button>
        <a href="index.php?controller=preguntas&action=create">Crear Pregunta</a>
    </button>
    <?php if (count($preguntas) > 0): ?>
        ...
        <nav aria-label="Paginación de notas" class="">
            <ul class="pagination justify-content-center">
                ...
                <?php if ($paginas[0] > 1): ?>
                    <li class="page-item">
                        <a class="page-link" href="index.php?controller=preguntas&action=list&id_tema=<?= $tema["id"] ?>&page=<?= $paginas[0] - 1; ?>">Anterior</a>
                    </li>
                <?php else: ?>
                ...
                <?php for ($i = 1; $i <= $paginas[1]; $i++): ?>
                    <li class="page-item <?= ($i == $paginas[0]) ? 'active' : ''; ?>">
                        <a class="page-link" href="index.php?controller=preguntas&action=list&id_tema=<?= $tema["id"] ?>&page=<?= $i; ?>"><?= $i; ?></a>
                    </li>
                <?php endfor; ?>
                ...
                <?php if ($paginas[0] < $paginas[1]): ?>
                    <li class="page-item">
                        <a class="page-link" href="index.php?controller=preguntas&action=list&id_tema=<?= $tema["id"] ?>&page=<?= $paginas[0] + 1; ?>">Siguiente</a>
                    </li>
                <?php else: ?>
                    <li class="page-item disabled">

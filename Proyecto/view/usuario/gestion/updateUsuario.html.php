...
<div class="user-dashboard">
    <div class="panelLateralMovil">
        ...
        <?php if ( $usuarioEdit["rol"] == "admin" || $usuarioEdit["rol"] == "gestor" ): ?>
            <button class="btn btnCrear btnPanelLateral" id="btnPanelControl">Panel de Control</button>
        <?php endif; ?>
    </div>
    <div class="user-profile-section">
        <h1 class="user-name"><?php echo htmlspecialchars($usuarioEdit['nombre'] . ' ' . $usuarioEdit['apellido']); ?></h1>
        ...
        <form action="index.php?controller=usuario&action=updateFoto" method="post" id="photoForm" enctype="multipart/form-data" class="profile-photo-form">
            <input type="hidden" name="idUsuario" id="idUsuario" value="<?php echo htmlspecialchars($usuarioEdit['id']); ?>">

            <div class="profile-photo-container">
                <img class="profile-photo" id="userImage" src="<?php echo isset($usuarioEdit['foto_perfil']) && $usuarioEdit['foto_perfil'] != "" ? $usuarioEdit['foto_perfil'] : 'assets/img/fotoPorDefecto.png'; ?>" alt="Foto de perfil">
    ...
    <div class="user-data-section">
        ...
        <form action="index.php?controller=usuario&action=update" method="post" id="userDataForm" class="user-data-form">
            ...
            <div class="form-field">
                ...
                <input disabled type="text" name="nombre" id="txtNombreEdit" class="field-input" value="<?php echo htmlspecialchars($usuarioEdit['nombre'] ?? ''); ?>" />
            </div>
            <div class="form-field">
                <label for="apellido" class="field-label">Apellido:</label>
                <input disabled type="text" name="apellido" id="txtApellidoEdit" class="field-input" value="<?php echo htmlspecialchars($usuarioEdit['apellido'] ?? ''); ?>" />
            </div>
            <div class="form-field">
                <label for="username" class="field-label">Usuario:</label>
                <div class="input-with-button">
                    <input disabled type="text" name="username" id="txtUsernameEdit" class="field-input" value="<?php echo htmlspecialchars($usuarioEdit['username'] ?? ''); ?>" />

                </div>
            </div>
            <div class="form-field">
                <label for="email" class="field-label">Email:</label>
                <input disabled type="text" name="email" id="txtEmailEdit" class="field-input" value="<?php echo htmlspecialchars($usuarioEdit['email'] ?? ''); ?>" />
            </div>
